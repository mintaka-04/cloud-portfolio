  ...
  sections: [
    ...
    section(4, '판단 및 이유', `
      ...
      ${text([
        ...
        '이에 따라 <strong>단일 Worker 구조의 처리 한계</strong>가 드러난 것으로 판단하였습니다. 또한 과부하 시점에는 CloudWatch Agent가 CPU 및 메모리 로그를 확보하지 못하게 되므로, 정확한 병목 분석에는 한계가 있다는 점도 확인하였습니다.',
      ])}
    `),
    ...
    section(5, '개선 방향', `
      ${text([
        '현재 구조에서는 AI 요청이 단일 Worker에 집중되어 병목이 발생하며, 일정 수준 이상의 부하에서 시스템이 안정적으로 동작하지 못하는 한계를 확인했습니다. 따라서 다음 버전에서는 <strong>요청을 안정적으로 처리할 수 있는 구조</strong>로 개선할 필요가 있다고 판단했습니다.',
